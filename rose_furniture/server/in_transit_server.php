...
while ($row = mysqli_fetch_assoc($sql))
{
  ...
  $sql2 = mysqli_query($conn, "SELECT order_text FROM tbl_order_process WHERE order_text = 'In Transit' AND cart_id = '".$row['cart_id']."' AND status = 1 ");
  ...
  $sql5 = mysqli_query($conn, "SELECT order_text, order_remarks FROM tbl_order_process WHERE id = '".$lastId['last_id']."' ");
