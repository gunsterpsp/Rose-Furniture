  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-2">
          <label for="">Select a courier for transit</label>
...
<script>
  ...
  $(document).on("click", ".claimBtn", function() {
    ...
    const claimBtn = $(".claimBtn").val();

    Swal.fire({
      title: 'This is not ireversable, Do you want to approve?',
      showDenyButton: true,
      confirmButtonText: 'Yes',
    }).then((result) => {
      /* Read more about isConfirmed, isDenied below */
      if (result.isConfirmed) {
        Swal.fire('Claiming has been approved!', '', 'success');
        $.ajax({
          type: 'POST',
  ...
  $(document).on("click", ".approve", function() {
    ...
    if (rider_id == 0 || rider_id == "") return alert("Please select a courier")

    Swal.fire({
      title: 'This is not ireversable, Do you want to confirm this rider?',
      showDenyButton: true,
      confirmButtonText: 'Yes',
    }).then((result) => {
      /* Read more about isConfirmed, isDenied below */
      if (result.isConfirmed) {
        Swal.fire('Rider has been set!', '', 'success');
        $.ajax({
          type: 'POST',
          url: '../server/function_transit.php',
          data: {
  ...
  $(document).on("click", ".approve2", function() {
    ...
    if (rider_id2 == 0 || rider_id2 == "") return alert("Please select a rider")
